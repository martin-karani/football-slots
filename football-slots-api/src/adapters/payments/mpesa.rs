...
use crate::domain::models::payment::{MpesaBalanceQuery, PaymentProvider, PaymentTransaction, UnmatchedDeposit};
use crate::ports::payments::{
    DepositInitiation, PaymentProviderPort, ProviderEvent, ProviderSubmission, TxLookup, WithdrawalInitiation,
};
...
impl MpesaAdapter {
    ...
    async fn get_access_token(&self) -> Result<String, DomainError> {
        ...
        let url = format!("{}/oauth/v1/generate", self.config.base_url);
    ...
    pub fn parse_safaricom_balance_string(raw: &str) -> (Option<i64>, Option<i64>, Option<i64>, Option<i64>) {
    ...
    async fn send_stk_push(
        ...
        let access_token = match self.get_access_token().await {
            Ok(t) => t,
            Err(e) => return ProviderSubmission::Rejected(format!("Failed to get access token: {}", e)),
        };
        ...
        let payload = serde_json::json!({
            ...
            "CallBackURL": format!("{}/api/v1/payments/webhooks/mpesa/stk_callback", self.app_base_url),
        ...
        match response {
            Ok(resp) => {
                let body = match resp.json::<serde_json::Value>().await {
                    Ok(b) => b,
                    Err(e) => return ProviderSubmission::Unknown(format!("Failed to parse STK response: {}", e)),
                };
                ...
                let desc = body["ErrorMessage"].as_str().unwrap_or("STK Push rejected");
                ProviderSubmission::Rejected(desc.to_string())
            }
            Err(e) if e.is_timeout() || e.is_connect() => {
                ProviderSubmission::Unknown(format!("STK Push network error: {}", e))
            }
            Err(e) => ProviderSubmission::Unknown(format!("STK Push error: {}", e)),
        }
    }
    ...
    async fn send_b2c_payment(&self, tx: &PaymentTransaction) -> ProviderSubmission<WithdrawalInitiation> {
        let access_token = match self.get_access_token().await {
            Ok(t) => t,
            Err(e) => return ProviderSubmission::Rejected(format!("Failed to get access token: {}", e)),
        };

        let conversation_id = tx.provider_conversation_id.as_deref().unwrap_or("");
        ...
        let payload = serde_json::json!({
            ...
            "QueueTimeOutURL": format!("{}/api/v1/payments/webhooks/mpesa/b2c_timeout", self.app_base_url),
            "ResultURL": format!("{}/api/v1/payments/webhooks/mpesa/b2c_result", self.app_base_url),
        ...
        match response {
            Ok(resp) => {
                let body = match resp.json::<serde_json::Value>().await {
                    Ok(b) => b,
                    Err(e) => return ProviderSubmission::Unknown(format!("Failed to parse B2C response: {}", e)),
                };
    ...
    pub async fn resolve_user_from_c2b_ref(&self, bill_ref: &str) -> DomainResult<Option<uuid::Uuid>> {
        if bill_ref.is_empty() {
            return Ok(None);
        }

        // Try as UUID first
        if let Ok(user_id) = bill_ref.parse::<uuid::Uuid>() {
    ...
    pub async fn register_c2b_urls(&self) -> DomainResult<()> {
        ...
        let payload = serde_json::json!({
            ...
            "ConfirmationURL": format!("{}/api/v1/payments/webhooks/mpesa/c2b_confirmation", self.app_base_url),
            "ValidationURL": format!("{}/api/v1/payments/webhooks/mpesa/c2b_validation", self.app_base_url)
        });
    ...
    pub async fn register_pull_transactions(&self) -> DomainResult<()> {
        ...
        let payload = serde_json::json!({
            ...
            "CallBackURL": format!("{}/api/v1/payments/webhooks/mpesa/c2b_confirmation", self.app_base_url)
        });
    ...
    pub async fn reconcile_missed_transactions(&self) -> DomainResult<i64> {
        ...
        loop {
            ...
            for tx in &transactions {
                ...
                if self.payment_repo.find_by_receipt("mpesa", trans_id).await.ok().flatten().is_some() {
                    continue;
                }
    ...
    pub async fn query_account_balance(&self) -> DomainResult<MpesaBalanceQuery> {
        ...
        let payload = serde_json::json!({
            ...
            "QueueTimeOutURL": format!("{}/api/v1/payments/webhooks/mpesa/accountbalance_timeout", self.app_base_url),
            "ResultURL": format!("{}/api/v1/payments/webhooks/mpesa/accountbalance_result", self.app_base_url)
        });
        ...
        self.mpesa_ops_repo.create_balance_query(&originator_id).await
    }
    ...
    pub async fn process_account_balance_callback(&self, payload: serde_json::Value) -> DomainResult<()> {
        ...
        if result_code == 0 {
            ...
            self.mpesa_ops_repo.update_balance_result(
    ...
    pub async fn business_pay_bill(
        ...
        let payload = serde_json::json!({
            ...
            "Remarks": remarks,
            "QueueTimeOutURL": format!("{}/api/v1/payments/webhooks/mpesa/b2b_timeout", self.app_base_url),
            "ResultURL": format!("{}/api/v1/payments/webhooks/mpesa/b2b_result", self.app_base_url),
        });
    ...
    pub async fn list_unmatched_deposits(&self, limit: i64, offset: i64) -> DomainResult<Vec<UnmatchedDeposit>> {
        self.payment_repo.list_unmatched_deposits(limit, offset).await
    }

    pub async fn resolve_unmatched_deposit(&self, deposit_id: uuid::Uuid, user_id: uuid::Uuid) -> DomainResult<UnmatchedDeposit> {
        self.payment_repo.resolve_unmatched_deposit(deposit_id, user_id).await
    }
}
...
impl PaymentProviderPort for MpesaAdapter {
    ...
    async fn process_webhook(&self, webhook: &str, payload: serde_json::Value) -> DomainResult<Option<ProviderEvent>> {
        let payload_clone = payload.clone();
        ...
        match webhook {
            "stk_callback" => {
                ...
                let result_code = payload_clone["Body"]["stkCallback"]["ResultCode"].as_i64().unwrap_or(1) as i32;
                let result_desc = payload_clone["Body"]["stkCallback"]["ResultDesc"].as_str().map(String::from);

                if result_code == 0 {
                    let amount_minor = payload_clone["Body"]["stkCallback"]["CallbackMetadata"]["Item"]
                        .as_array()
            ...
            "b2c_result" | "b2c_timeout" => {
                let result = &payload_clone["Result"];
                let conversation_id = result["OriginatorConversationID"].as_str().unwrap_or("");
                let result_code = result["ResultCode"].as_i64().unwrap_or(1) as i32;
            ...
            "c2b_confirmation" => {
                ...
                // Try to resolve user
                let user_id = self.resolve_user_from_c2b_ref(bill_ref).await?;
